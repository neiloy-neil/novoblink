...
export default function NewSmartCollectionPage() {
  ...
  const updateRule = (i: number, patch: Partial<Rule>) => setRules(r => r.map((rule, idx) => idx === i ? { ...rule, ...patch } : rule))

  const handleSave = async () => {
    if (!form.name || !form.slug) { toast.error("Name and slug are required"); return }
    setSaving(true)
    try {
      const res = await fetch("/api/admin/smart-collections", {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold">New Smart Collection</h1>
      ...
      <div className="space-y-4 border rounded-lg p-5">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="space-y-1">
            ...
            <Input value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value, slug: e.target.value.toLowerCase().replace(/\s+/g, "-").replace(/[^a-z0-9-]/g, "") }))} placeholder="Under ৳1000" />
          </div>
          <div className="space-y-1">
      ...
      <div className="flex gap-3">
        <Button onClick={handleSave} disabled={saving}>{saving ? "Saving…" : "Create Collection"}</Button>
